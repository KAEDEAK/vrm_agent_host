...
| `lookAtBone` | 指定ボーンへ視線を向ける | `?target=vrm&cmd=lookAtBone&bone=LeftHand` |
| `lookAtCamera` | カメラ方向へ視線を向ける | `?target=vrm&cmd=lookAtCamera` |
| `gazeControl` | 視線制御の有効/無効 | `?target=vrm&cmd=gazeControl&enabled=true` |
| `lookAtHead` | (旧) 視線制御の有効/無効 | `?target=vrm&cmd=lookAtHead&enable=false` |
...
- [Updated] VRM10の標準的な視線制御システム（Vrm10RuntimeLookAt）を使用するように更新しました。
- [Added] 新しい `gazeControl` コマンドで視線制御の有効/無効を切り替えられます（`lookAtHead` は後方互換性のため維持）。
- [Improved] `lookAtBone` と `lookAtCamera` がターゲット追跡モードで動作するようになりました。
- [Note] VRM10では左右の目の個別制御（`eye` パラメータ）はサポートされていません。
