...
| `look` | 視線角度を直接指定 | `?target=vrm&cmd=look&mode=deg&yaw=30&pitch=-15&eye=both` |
| `lookAtBone` | 指定ボーンへ視線を向ける | `?target=vrm&cmd=lookAtBone&bone=LeftIndexDistal&mode=deg&eye=left` |
| `lookAtCamera` | カメラ方向へ視線を向ける | `?target=vrm&cmd=lookAtCamera&mode=deg&eye=right` |
| `lookAtHead` | LookAtHead コンポーネントの有効/無効 | `?target=vrm&cmd=lookAtHead&enable=false` |
...
- [Implemented] AGIA アニメーション再生中に `load` コマンドを送ると `busy` (409) を返します。
- [Implemented] シームレス遷移でも AGIA playback lock を利用し、VRM ロード競合を防ぎます。
- [Implemented] VRM ロード時には進行中のアニメーションコルーチンを停止し、MissingReferenceException を防ぎます。
- [Fixed] AGIAアニメーション完了検知を改善し、ロックカウンターが正常に0に戻るように修正しました。
- [Added] デバッグコマンドでロック状態の確認と強制解除が可能になりました。
- [Implemented] lookAtHead コマンドで LookAtHead を有効/無効化できるようになりました。
- [Implemented] VRMLookAtHead の UpdateType を LateUpdate に変更し、Animator に上書きされないようにしました。
- [Implemented] look/lookAtBone/lookAtCamera コマンドは VRMLookAtHead に yaw/pitch を伝達し、BlendShape モデルでも視線が反映されます。
...
| cmd | 概要 | 例 |
| --- | --- | --- |
| `reset` | アニメーションを初期化（ロックカウンターもリセット） | `?target=animation&cmd=reset` |
...
- [Fixed] `reset`コマンドでAGIAロックカウンターも同時にリセットされるようになりました。
- [Added] デバッグコマンドでロック状態の確認と強制解除が可能になりました。
- [Improved] アニメーション完了検知を1.5秒間隔の監視に最適化し、30秒のタイムアウト機能を追加しました。
