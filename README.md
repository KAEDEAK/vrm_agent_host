...
初回再生時にVRMAアニメーションの終了ポーズと次のアニメーションポーズを比較し、
Hipsボーンの位置・回転差分を記録します。以降同じVRMモデルで同じVRMAを再生すると、
キャッシュされたオフセットが自動的に適用され、ポーズのずれを補正します。
オフセットはメモリ上のみ保持され、設定ファイルでの永続化や無効化項目はありません。
座標ずれの主な原因はアニメーション間の姿勢差です。`autoPrepareSeamless`を有効にし、移動よりもシームレス切替で補正することを推奨します。
`autoPrepareSeamless` が true の場合、`play` コマンドで `seamless` パラメータを省略すると自動的にシームレス遷移が行われます。
...
| `waveplay_start` | Wave 再生リスナーの状態取得 | `?target=server&cmd=waveplay_start` |
| `waveplay_stop` | Wave 再生リスナー停止 | `?target=server&cmd=waveplay_stop` |
...
`POST /waveplay/` に RIFF/WAVE (mono 16bit 48kHz) を送信すると、アバター側で即座に再生されます。
