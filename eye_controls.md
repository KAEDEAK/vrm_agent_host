...
## [NotStarted] HTTP コマンド設計
| コマンド       | パラメータ                         | 説明                 |
|---------------|------------------------------------|----------------------|
| `look`         | `mode`, `yaw`, `pitch`            | 自由視線指定         |
| `lookAtBone`   | `bone`, `mode`                    | 骨ターゲット追従     |

#### [NotStarted] コマンド例
```text
/?target=vrm&cmd=look&mode=deg&yaw=30&pitch=-15
/?target=vrm&cmd=lookAtBone&bone=LeftIndexDistal&mode=norm
```
...
### [NotStarted] LookCommandHandler.cs
```csharp
using System;
using System.Net;
using UnityEngine;
...
### [NotStarted] LookAtBoneCommandHandler.cs
```csharp
using System;
using System.Net;
using UnityEngine;
