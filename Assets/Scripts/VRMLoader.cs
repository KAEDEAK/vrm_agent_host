...
public class VRMLoader : MonoBehaviour {
    ...
    public async Task ReloadVRMAsync(string fullPath) {
        ...
        try {
            ...
            if (VrmInstance != null)
            {
                // VRM10LookAtControllerを追加
                if (LoadedModel.GetComponent<VRM10LookAtController>() == null)
                {
                    LoadedModel.AddComponent<VRM10LookAtController>();
                    Debug.Log("[VRMLoader] VRM10LookAtController added to loaded model");
                }
                
                // LookAtTargetTypeをSpecifiedTransformに設定
                VrmInstance.LookAtTargetType = VRM10ObjectLookAt.LookAtTargetTypes.SpecifiedTransform;
                
                // メインカメラをターゲットに設定
                if (Camera.main != null)
                {
                    VrmInstance.LookAtTarget = Camera.main.transform;
                    Debug.Log("[VRMLoader] VRM10 LookAt target set to main camera");
                }
                
                // UpdateTypeをLateUpdateに設定
                VrmInstance.UpdateType = Vrm10Instance.UpdateTypes.LateUpdate;
                Debug.Log("[VRMLoader] VRM10 UpdateType set to LateUpdate");
            }
