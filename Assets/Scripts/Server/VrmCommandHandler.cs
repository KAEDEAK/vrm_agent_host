...
public class VrmCommandHandler : HttpCommandHandlerBase {
    ...
    private static readonly string[] AllowedCommands = {
        ...
        "look", "lookAtBone", "lookAtCamera", "lookAtHead", "gazeControl",
    ...
    public override async void HandleCommand(HttpListenerContext context, NameValueCollection query) {
        ...
        switch (cmd) {
            ...
            case "look": {
                    ...
                    // VRM10LookAtControllerを使用
                    VRM10LookAtController.SetGlobalLookRotation(yawDeg, pitchDeg);
                    responseData.status = 200;
            ...
            case "lookAtBone": {
                    ...
                    // HumanBodyBonesの有効な値をリストアップ
                    var validBones = new List<string> {
                    ...
                    if (!Enum.TryParse<HumanBodyBones>(boneName, out var boneEnum)) {
                        responseData.status = 400;
                        responseData.message = $"無効な bone 名です。有効な値: {string.Join(", ", validBones)}";
                        break;
                    }
                    ...
                    if (targetTransform == null) {
                        responseData.status = 500;
                        responseData.message = $"Bone transform not found for {boneName}.";
                        break;
                    }
                    ...
                    responseData.status = 200;
                    responseData.message = $"lookAtBone applied to {boneName}";
                    break;
                }
            case "lookAtCamera": {
                    ...
                    // VRM10の視線制御でカメラをターゲットに設定
                    VRM10LookAtController.SetGlobalLookAtTarget(camera.transform);
            ...
            case "gazeControl": {
                    // 新しいコマンド名、パラメータ名は "enabled"
                    string enabledParam = GetQueryParam(query, "enabled", null);
                    if (string.IsNullOrEmpty(enabledParam)) {
                        responseData.status = 400;
                        responseData.message = i18nMsg.ERROR_ENABLE_PARAM_MISSING;
                        break;
                    }
                    if (!bool.TryParse(enabledParam, out bool enabled)) {
                        responseData.status = 400;
                        responseData.message = i18nMsg.ERROR_ENABLE_PARAM_INVALID;
                        break;
                    }

                    // VRM10の場合、Vrm10Instanceの視線制御を有効/無効にする
                    var vrm10Instance = _vrmLoader?.VrmInstance;
                    if (vrm10Instance != null) {
                        if (enabled) {
                            // 視線制御を有効にする（デフォルトでカメラを見る）
                            ...
                            Debug.Log("[VrmCmd] VRM10 gaze control enabled (looking at camera)");
                        } else {
                            // 視線制御を無効にする（正面を向く）
                            vrm10Instance.LookAtTargetType = VRM10ObjectLookAt.LookAtTargetTypes.YawPitchValue;
                            // 正面を向くように設定
                            var lookAtController = vrmInstanceObj.GetComponent<VRM10LookAtController>();
                            if (lookAtController != null) {
                                lookAtController.SetLookRotation(0f, 0f);
                            }
                            Debug.Log("[VrmCmd] VRM10 gaze control disabled (looking forward)");
                        }
                        
                        responseData.status = 200;
                        responseData.message = $"gaze control {(enabled ? "enabled" : "disabled")}";
                    } else {
                        responseData.status = 500;
                        responseData.message = "VRM instance not found";
                    }
                    break;
                }
            case "lookAtHead": {
                    // 後方互換性のために残す（gazeControlと同じ処理）
                    ...
                    if (vrm10Instance != null) {
                        if (enable) {
                            ...
                            Debug.Log("[VrmCmd] VRM10 LookAt enabled (looking at camera) - via lookAtHead");
                        } else {
                            // 視線制御を無効にする（正面を向く）
                            vrm10Instance.LookAtTargetType = VRM10ObjectLookAt.LookAtTargetTypes.YawPitchValue;
                            // 正面を向くように設定
                            ...
                            Debug.Log("[VrmCmd] VRM10 LookAt disabled (looking forward) - via lookAtHead");
                        }
